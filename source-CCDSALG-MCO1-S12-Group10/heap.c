...
extern char heapSortedNames[8][50];

void heapAlgo(int n, long long int *OutCnt, long long int *InCnt, Record *arr, double startTime, double endTime, double executionTime)   //the "body" of the heap program
{
    ...
    for(i = 0; i < 8; i++)
    {
        ...
        heapSort(arr, n, OutCnt, InCnt);   //the "heart" of the insertion sort algorithm
        endTime = currentTimeMillis();
        executionTime = (endTime - startTime) / 1000;   //converts the execution time into seconds

        //printToFile(arr, n, heapSortedNames[i]);   //prints the sorted data set into a file, disabled by default
    ...
    printf("HeapSortData.txt file generated...\n\n");
    fclose(wp);
}
