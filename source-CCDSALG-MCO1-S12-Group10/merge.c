...
extern char mergeSortedNames[8][50];

void mergeAlgo(int n, long long int *OutCnt, long long int *InCnt, Record* arr, double startTime, double endTime, double executionTime)  //the "body" of the merge program
{
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        mergeSort(arr, n, OutCnt, InCnt);   //the "heart" of the merge sort algorithm
        endTime = currentTimeMillis();
        executionTime = (endTime - startTime) / 1000;   //converts the execution time into seconds

        //printToFile(arr, n, mergeSortedNames[i]); //prints the sorted data set into a file, disabled by default
    ...
    printf("MergeSortData.txt file generated...\n\n");
    fclose(wp);
}
